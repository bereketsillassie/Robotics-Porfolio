...
def track_face(info, pid_yaw, pid_fb, pError_yaw, iError_yaw, dError_yaw,
    ...
    max_iError_yaw = 10000  # Adjust this
    ...
    max_speed_yaw = 30.0
    speed_yaw = np.clip(speed_yaw, -max_speed_yaw, max_speed_yaw)  # Limit steering angle for new design
    ...
    side_side_command_temp = int(64 + (speed_yaw / max_speed_yaw) * 62)
    side_side_command = max(0, min(126, side_side_command_temp))

    # Map desired speed to front-back command (0-126, with 64 as stop)
    front_back_command_temp = int(64 + (speed_fb / max_speed_fb) * 62)
    front_back_command = max(0, min(126, front_back_command_temp))
        ...
        if not command_queue.empty():
            ...
            if command == 'face_tracking':
                ...
                if 'detections' in detection and len(detection['detections']) > 0:
                    ...
                    # If no detection received then  stop the robot
