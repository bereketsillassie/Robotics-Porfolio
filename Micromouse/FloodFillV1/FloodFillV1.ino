...
#define ROBOT_WIDTH_MM 105   // Distance between wheels (measure yours!)
...
// === Movement + PID Motor Constants ===
// Update with your actual constants/pins
...
void initMaze() {
  for (int r = 0; r < MAZE_SIZE; r++) {
    for (int c = 0; c < MAZE_SIZE; c++) {
      for (int d = 0; d < 4; d++) {
        maze[r][c].wall[d] = true;  // unknown walls treated as blocked
      }
  ...
  while (!q.empty()) {
    ...
    for (int dir = 0; dir < 4; dir++) {
      ...
      if (!maze[nr][nc].visited) {
        ...
        maze[nr][nc].distance = maze[row][col].distance + 1;
        q.push({nr, nc});
      }
    }
  }
...
// === Direction Selection ===
int getBestDirection(int row, int col) {
  int bestDir = -1;
  int minDist = 9999;
  ...
  for (int dir = 0; dir < 4; dir++) {
    ...
    if (nr >= 0 && nr < MAZE_SIZE && nc >= 0 && nc < MAZE_SIZE &&
        !maze[row][col].wall[dir]) {
      if (maze[nr][nc].distance < minDist) {
        minDist = maze[nr][nc].distance;
        bestDir = dir;
      }
    }
  }
...
void navigateToCenter() {
  while (!isCenter(robotRow, robotCol)) {
    // 1. Read current cell's walls from sensors
    updateWallsFromSensors();

    // 2. Recalculate distances using current known walls
    floodFill();

    // 3. Choose direction based on lowest distance neighbor
    int dir = getBestDirection(robotRow, robotCol);

    // 4. Move toward that direction
    if (dir != -1) {
      move(dir);
    } else {
...
// Replace these with your actual motor control code
void updatePD() {
...
void updateWallsFromSensors() {
  ...
  // Also update the neighbor cells' opposite walls if they are in bounds
...
void loop() {
  // Nothing — run everything in setup for now
}
