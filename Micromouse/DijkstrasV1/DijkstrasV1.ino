...
vector<pair<int,int>> dijkstraPlan(int startR, int startC) {
  ...
  while (!open.empty()) {
    ...
    for (int dir = 0; dir < 4; dir++) {
      ...
      float gNew = g[cur.r][cur.c] + 1.0f;  // uniform cost per move
      if (gNew < g[nr][nc]) {
        g[nr][nc] = gNew;
        parentR[nr][nc] = cur.r;
        parentC[nr][nc] = cur.c;
        open.push({nr, nc, g[nr][nc]});
      }
    }
  }
...
void navigateToCenter() {
  while (!isCenter(robotRow, robotCol)) {
    ...
    vector<pair<int,int>> path = dijkstraPlan(robotRow, robotCol);
    if (path.empty()) {
      break;
    }
